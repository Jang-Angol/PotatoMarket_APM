...
<?php
    ...
    if(!isset($_GET["server"])||empty($_GET["server"])){
        $sql = "SELECT ITEM_TB.no, ITEM_TB.user_no, ITEM_TB.trade_type, ITEM_TB.trade_state, ITEM_TB.server, ITEM_TB.title, ITEM_TB.price, ITEM_TB.date FROM ITEM_TB";
        $count_sql = "SELECT COUNT(*) as item_count FROM ITEM_TB";

        //FOR DETAIL SEARCH
        if(!empty($_GET["item_opt1"])||!empty($_GET["item_opt2"])||!empty($_GET["item_opt3"])){
            $sql = $sql." LEFT JOIN ITEM_OPT_TB ON ITEM_TB.no = ITEM_OPT_TB.item_no WHERE ITEM_TB.trade_type = 2";
            $count_sql = $count_sql." LEFT JOIN ITEM_OPT_TB ON ITEM_TB.no = ITEM_OPT_TB.item_no WHERE ITEM_TB.trade_type = 2";
            ...
            $sql = $sql." WHERE ITEM_TB.trade_type = 2";
            $count_sql = $count_sql." WHERE ITEM_TB.trade_type = 2";
        }
        ...
        //FOR DETAIL SEARCH
        if(!empty($_GET["item_opt1"])||!empty($_GET["item_opt2"])||!empty($_GET["item_opt3"])){
            $sql = $sql." LEFT JOIN ITEM_OPT_TB ON ITEM_TB.no = ITEM_OPT_TB.item_no WHERE ITEM_TB.trade_type = 2 AND ITEM_TB.server = $item_server";
            $count_sql = $count_sql." LEFT JOIN ITEM_OPT_TB ON ITEM_TB.no = ITEM_OPT_TB.item_no WHERE ITEM_TB.trade_type = 2 AND ITEM_TB.server = $item_server";
            if(!empty($_GET["item_opt1"])){
                $sql = $sql." AND ITEM_OPT_TB.opt LIKE '%$_GET[item_opt1]%'";
                $count_sql = $count_sql." AND ITEM_OPT_TB.opt LIKE '%$_GET[item_opt1]%'";
            }
            ...
            $sql = $sql." WHERE ITEM_TB.trade_type = 2 AND ITEM_TB.server = $item_server";
            $count_sql = $count_sql." WHERE ITEM_TB.trade_type = 2 AND ITEM_TB.server = $item_server";
        }
